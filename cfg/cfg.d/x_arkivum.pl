...
$c->{plugins}->{"Storage::ArkivumStorage"}->{params}->{mount_path} = "/usr/share/eprints/archives";
...
$c->{plugins}->{"Storage::ArkivumStorage"}->{params}->{auth_url} = "";
$c->{plugins}->{"Event::Arkivum"}->{params}->{auth_url} = "";
